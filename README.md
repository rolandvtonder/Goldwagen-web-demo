...
**The logo** has a solid white background (not transparent) and is only 197×97px, so
it is soft on high-density screens. It sits in a deliberate white plate
(`.logo-plate`) so it reads as intentional rather than as a stray white box. Send a
transparent SVG or a larger PNG and this gets noticeably sharper — you can then drop
the `.logo-plate` wrapper.
